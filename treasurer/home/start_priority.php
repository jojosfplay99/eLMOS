...
$query = mysqli_query($con,"SELECT * FROM que_treasurer WHERE date_sched LIKE '$dated' AND status LIKE 'SERVING' AND clerkid = '$clerkid'");
if(mysqli_num_rows($query) == 0){
    mysqli_query($con,"UPDATE que_treasurer SET status = 'SERVING' , clerkid = '$clerkid' WHERE id = '$id'");  
}
else{
    ...
    mysqli_query($con,"UPDATE que_treasurer SET status = 'DONE' WHERE id = '$id2'");      
}
