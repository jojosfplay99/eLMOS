...
$(document).ready(function(){
    ...
    var table1 = new DataTable('#example', {                        
        ajax: 'classification2/server1.php',
        processing: true,
        serverSide: false,         
        order:[0,'asc'],
        columnDefs: [
            ...
            {              
                ...
                action: function ( e, dt, node, config ){
                    ...
                    $( '#add_subclass_select' ).select2( {
                        ...
                        ajax: {
                            url: "classification2/classification_select.php",
                            type: "post",
                            dataType: 'json',
                            delay: 250,
                    ...
                    $('#submit_new_subclass').on('submit', function(e){
                        ...
                        $.ajax({
                            method: "POST",
                            url: "classification2/add_subclass.php",            
                            data:$(this).serialize(),
                        ...
                        action: function ( e, dt, node, config ){
                            ...
                            function setClassCode(total_length,counter,values,classname){  
                                if(counter < total_length){
                                    ...
                                    $.ajax({
                                        method: "POST",
                                        url: "classification2/set_classification.php",            
    ...
    $('#example tbody').on('click', '.btn-primary', function(){
        ...
        $('#edit_subclass_select').val(data[1]).trigger('change')
        $('#edit_subclass_code').val(data[2])
        $('#edit_subclass_mode').val(data[3]).trigger('change')
        $('#edit_subclass_description').val(data[4])
        $('#edit_subclass_lowrange').val(data[5])
        $('#edit_subclass_uprange').val(data[6])
        $('#edit_subclass_value').val(data[7])
        $('#edit_subclass_type').val(data[8])
        $('#edit_subclass_status').val(data[9])
        if(data[1] != null){
            var select_id = data[1];
            $.ajax({
                method: "POST",
                url: "classification2/classification_select.php",            
                data:{searchTerm2:data[1]},
                success: function(data) {  
        ...
        $('#submit_edit_subclass').on('submit', function(e){
            ...
            $.ajax({
                method: "POST",
                url: "classification2/edit_subclass.php",            
                data:$(this).serialize(),
                success: function(data) {  
                    Swal.fire({
                        icon:'success',
    ...
    $('#example tbody').on('click', '.btn-danger', function(){
        ...
        if (result.isConfirmed) {
            $.ajax({
                method: "POST",
                url: "classification2/delete_subclass.php",            
                data:{id:data[0]},
                success: function(data) { 
                    Swal.fire({
                        icon:'success',
