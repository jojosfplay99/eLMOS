...
$status2 = mysqli_escape_string($con,$_POST['status2']);
$clerkid = mysqli_escape_string($con,$_POST['clerkid']);
$floor_item = mysqli_escape_string($con,$_POST['floor_item']);
$tdno_classification = mysqli_escape_string($con,$_POST['tdno_classification']);
...
$depreciation_level = mysqli_escape_string($con,$_POST['depreciation_level']);
$depreciation_value = mysqli_escape_string($con,$_POST['depreciation_value']);
...
mysqli_query($con,"INSERT INTO propertyfaasdb2(faasid, propertyid, tdno, classification, sub_classification, area, area_mode, per_sqm, basic_value, depreciation_level, depreciation_value, base_market, marketvalue, adjustment_level, actualuse, assessedlevel, assessedvalue, floor_item, status, clerkid) VALUES('$faasid','$propertyid','$tdno_classification','$classification_name','$sub_classification','$area','$area_mode','$per_sqm','$unit_value','$depreciation_level','$depreciation_value','$basic_value','$marketvalue','$adjustment_level','$actual_use','$assessment_level','$assessed_value','$floor_item','$status2','$clerkid')") or die(mysqli_error($con))
