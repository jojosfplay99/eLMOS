$(document).ready(function(){
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('#previous_assval').text($.number(previous_assval,2));
            
            var regex = /(<([^>]+)>)/ig
            var result = memoranda.replace(regex, "");
            var result2 = annotation.replace(regex, "");
            
    
            
            if(result.length > 300 || result2.length > 300){                
                $("#content_2next").removeAttr('style').css("display","show");
                $('#memoranda1').html(result.slice(0,300)+'...(Additional details at the back)');
                $('#annotation1').html(result2.slice(0,300)+'...(Additional details at the back)');          
            }else{
                $('#memoranda1').html(result);
                $('#annotation1').html(result2); 
            }
            ...
            $('#memoranda2').html(memoranda);            
            $('#annotation').text(annotation);            
            $('#annotation2').html(annotation);
            $('#status').text(status);
        ...
        success: function (response) {
            ...
            if(response.count == 0) {
                ...
                $.each(response, function(index, value){
                    ...
                    $('#classvalue').append('<tr style="font-size:12px;"><td class="classifications">'+classification+'</td><td class="classifications">'+per_sqm+' SQM</td><td class="classifications">'+sub_classification+'</td><td class="classifications">₱ '+marketvalue+'</td><td class="classifications">₱ '+assessedvalue+'</td></tr>');                                            
                }); 
                
                $('#classvalue').append('<tr style="font-size:12px;"><td colspan="4" class="classifications">TOTAL</td><td class="classifications">₱ '+$.number( total_assval, 2 )+'</td></tr>');                        
    ...
    $.ajax({
        type: "POST",
        url: "check_coowners.php",
        data:{id:taxdec_id},
        dataType: "json",
        success: function (response) {        
            if(response[0].count == 0){                
                $("#additional_page").remove();
                $("#content_next").remove();                 
            }else{                
                $("#content_next").removeAttr('style').css("display","show");                
                $('#additional_page').css('page-break-after','always')
                $('#content_next').append('<table id="add_coowner_tables" class="table table-bordered table-striped table-responsive"><tr><th>CO-OWNER</th><th>TIN</th><th>TEL</th><th>ADDRESS</th></tr></table>')                        
                $.each(response, function(index, value){
                    $('#add_coowner_tables').append('<tr><td>'+response[index].ownername+'</td><td>'+response[index].tin+'</td><td>'+response[index].tel+'</td><td>'+response[index].address+'</td></tr>')                        
                });  
